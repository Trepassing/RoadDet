...
def plot_one_box(x, im, c=0, color=(128, 128, 128), label=None, line_thickness=3, mosaic=False, start=5):
    # Plots one bounding box on image 'im' using OpenCV
    assert im.data.contiguous, 'Image not contiguous. Apply np.ascontiguousarray(im) to plot_on_box() input image.'
    tl = line_thickness or round(0.002 * (im.shape[0] + im.shape[1]) / 2) + 1  # line/font thickness
    # insert by zhinanzhang
    if mosaic and c > start:
        do_mosaic(im, int(min(x[0],x[2])), int(min(x[1],x[3])), int(abs(x[2]-x[0]))+tl, int(abs(x[1]-x[3]))+tl)
...
def do_mosaic(frame, x, y, w, h, neighbor=9):
    ...
    """
    if neighbor > w / 2 or neighbor > h / 2:
        neighbor = 5
