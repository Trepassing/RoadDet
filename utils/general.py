...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300, multi_confs={}):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            if multi_conf:
                res = x[0].unsqueeze(0)
                for c in multi_confs:
                    each_thres = multi_confs[c]
                    each_inds = torch.where(j==c)[0]
                    each_conf, each_box, each_class = conf[each_inds], box[each_inds], j[each_inds]
                    each_x = torch.cat((each_box, each_conf, each_class.float()), 1)[each_conf.view(-1) > each_thres]
                    res = torch.cat((res, each_x), 0)
                x = res[1:]
            else:
...
def box_iou_array(box1, box2, eps=1e-7):
    # https://github.com/pytorch/vision/blob/master/torchvision/ops/boxes.py
    """
    Return intersection-over-union (Jaccard index) of boxes.
    Both sets of boxes are expected to be in (x1, y1, x2, y2) format.
    ...
    inter = np.clip(inter,0,999999999999999999)
    inter = np.prod(inter,axis=2)
    
    iou = inter / (area1[:, None] + area2 - inter)
    # inds = np.where((inter == area1) | (inter==area2))
    # iou[inds] = 0.9
    # return inter / (area1[:, None] + area2 - inter)
    return iou

def combine_box(predn, iou_th, classes=[1]):
    """
    process nms boxes from each class 
    """
    assert isinstance (classes, list) and isinstance(classes[0], int)
    if isinstance(predn, torch.Tensor):
        predn = predn.cpu().numpy()
    predn0 = predn.copy()
    # (predn[:, 5]==1) | (predn[:, 5]==3)
    for c in classes:
        dets = predn0[predn0[:, 5]==c]
        # if len(dets)==1:
        #     continue
        predn0 = predn0[predn0[:, 5]!=c]
        scores = dets[:, 4]
        # descending
        ...
        while order.size > 0:
            i = order[0]
            # cal ious
            ovr = box_iou_array(dets[[i]], dets[order[1:]])[0] # iou matrix
            inds = np.where(ovr <= iou_th)[0]
            cinds = np.where(ovr > iou_th)[0]
            if len(dets[cinds]) != 0:
                new_box = combine(dets[i], dets[order[cinds+1]], c)
                predn0 = np.vstack((predn0, new_box))
            else:
                predn0 = np.vstack((predn0, dets[i]))
            # 将order序列更新，由于前面得到的矩形框索引要比矩形框在原order序列中的索引小1，所以要把这个1加回来
            order = order[inds + 1]
    return predn0

def combine(box, boxes, c):
    boxes = np.vstack((boxes, box))
    xx1 = np.min(boxes[:,0])
    yy1 = np.min(boxes[:,1])
    xx2 = np.max(boxes[:,2])
    yy2 = np.max(boxes[:,3])
    score = np.max(boxes[:,4])
    box = np.array([xx1,yy1,xx2,yy2,score,c])
    return box
...
def inside(box1,box2):
    ...
    b1_x1, b1_y1, b1_x2, b1_y2 = box1[0], box1[1], box1[2], box1[3]
    b2_x1, b2_y1, b2_x2, b2_y2 = box2[0], box2[1], box2[2], box2[3]
    inter = np.clip(np.minimum(b1_x2, b2_x2) - np.maximum(b1_x1, b2_x1), 0, 100000) * \
            np.clip(np.minimum(b1_y2, b2_y2) - np.maximum(b1_y1, b2_y1), 0, 100000)

    oinds = np.where(inter==box_area(box1))
    if len(oinds[0])!=0:
        return oinds, False
    else:
        inds = np.where(inter==box_area(box2))
        return inds, True

def remove_inside_box(dets, classes=[0, 1, 2, 3, 4]):
    '''
    remove the smaller box inside bigger box from each class    
    '''
    if len(dets) == 0:
        return dets
    ...
    for c in classes:
        boxes = dets[dets[:, 5]==c]
        while len(boxes)>0:
            res, cflag = inside(boxes[0],boxes[1:]) # return inside box index and whether cbox is inside box
            if len(res[0])!=0:
                if cflag:
                    boxes = np.delete(boxes, res[0]+1, axis = 0)
                else:
                    boxes = np.delete(boxes, 0, axis = 0)
            else:
                out = np.vstack((out, boxes[0]))
                boxes = np.delete(boxes, 0, axis = 0)
    return out[1:]
...
def get_roi_res_torch(dets, height, ratio, endind, xyxy=True):
    ...
    dets1 = detsn[torch.where(detsn[:, 5] > endind)[0]]
    dets2 = detsn[torch.where(detsn[:, 5] <= endind)[0]]
    inds = torch.where(dets2[:, 3] > height * (1 - ratio))[0]
    res = torch.cat((dets2[inds], dets1), 0)
    return res
