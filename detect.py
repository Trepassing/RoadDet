...
@torch.no_grad()
# insert by zhinanzhang  add modeltype roi_thres
def detect(modeltype=2,
           roi_thres=0.75,
    ...
    if modeltype==2:
        ...
        if len(names) == 5:
            print('-' * 25 + 'Liqing mode' + '-' * 25)
            multi_confs = { 0 : 0.2, 1 : 0.05, 2 : 0.05, 3 : 0.2, 4 : 0.1 }
        elif len(names) == 6:
            print('-' * 25  + 'shuini mode' + '-' * 25)
            multi_confs = { 0 : 0.1, 1 : 0.025, 2 : 0.05, 3 : 0.05, 4 : 0.05, 5 : 1.0 }
        else:
            print("MODEL ERROR")
            assert False
    if half:
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                detn = get_roi_res_torch(detn, height, roi_thres)
                ...
                for *xyxy, conf, cls in reversed(combine_predn):  # det combine_predn
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        # insert by zhinanzhang
                        if c > 4 and modeltype == 2:
                            plot_one_box(xyxy, im0, label=label, color=colors(c, True), line_thickness=line_thickness, mosaic=True)
                        else:
...
if __name__ == '__main__':
    ...
    # insert by zhinanzhang
    parser.add_argument('--modeltype', type=int, default=2, help='the type of model; V1 V2')
